...
export function activate(context: vscode.ExtensionContext) {
    ...
    let generateProtocolCommand = vscode.commands.registerCommand('strataregula.generateProtocol', async (uri?: vscode.Uri) => {
...
function initializeStatusBar(context: vscode.ExtensionContext) {
    ...
    const configWatcher = vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('strataregula.statusBar.enabled')) {
            const newEnabled = vscode.workspace.getConfiguration('strataregula').get<boolean>('statusBar.enabled', true);
...
function updateStatusBar(status: 'indexing' | 'ready' | 'error', fileCount: number) {
    if (!statusBarItem) {
        return;
    }
...
function startLanguageServer(context: vscode.ExtensionContext) {
    ...
    client.start().then(() => {
        ...
        client.onNotification('strataregula/indexingProgress', (params: any) => {
...
async function runStrataRegulaGenerateProtocol(uri?: vscode.Uri) {
    try {
        const activeEditor = vscode.window.activeTextEditor;
        const targetFile = uri ? uri.fsPath : activeEditor?.document.uri.fsPath;
...
function generateStatsWebViewHtml(statsData: any): string {
    return `<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
